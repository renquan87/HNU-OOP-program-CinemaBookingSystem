...
6. [装饰器模式 (Decorator Pattern) - 未使用](#装饰器模式-decorator-pattern---未使用)
7. [其他设计特性](#其他设计特性)
...
### 与装饰器模式的区别
- **继承模式**：编译时确定类型，静态扩展功能
- **装饰器模式**：运行时动态添加功能，可以多层装饰

---

## 装饰器模式 (Decorator Pattern) - 未使用
...
public class VIPSeat extends Seat {
    ...
    public VIPSeat(int row, int col, double basePrice) {
        super(row, col, basePrice + PRICE_PREMIUM); // 编译时确定价格
    }
}
```
...
public abstract class SeatDecorator extends Seat {
    ...
    public SeatDecorator(Seat seat) {
        super(seat.getRow(), seat.getCol(), seat.getBasePrice());
        this.seat = seat;
    }
...
// 具体装饰器示例
public class HeatedSeatDecorator extends SeatDecorator {
    private boolean heated;
    
    public HeatedSeatDecorator(Seat seat) {
        super(seat);
        this.heated = false;
    }
    ...
    public double getBasePrice() {
        ...
        if (heated) {
            basePrice += 5.0; // 动态添加加热功能价格
        }
        return basePrice;
    }
}

public class MassageSeatDecorator extends SeatDecorator {
    private boolean massageEnabled;
    private int massageMode;
    
    public MassageSeatDecorator(Seat seat) {
        super(seat);
        this.massageEnabled = false;
        this.massageMode = 2;
    }
    ...
    public double getBasePrice() {
        double basePrice = seat.getBasePrice();
        if (massageEnabled) {
            basePrice += massageMode * 3.0; // 动态添加按摩功能价格
        }
        return basePrice;
    }
}
...
#### 项目中选择继承模式的原因
1. **业务需求明确**：座位类型在业务上是固定的（VIP、折扣、普通）
2. **性能考虑**：继承模式没有装饰器的委托开销
3. **简单直接**：继承模式更容易理解和实现
4. **类型安全**：编译时就能确定座位类型
...
// 使用装饰器模式的示例
public Order createOrder(User user, Show show, List<String> seatIds, 
                        Map<String, List<String>> seatFeatures) {
    ...
    for (String seatId : seatIds) {
        ...
        // 根据用户选择的功能装饰座位
        if (seatFeatures != null && seatFeatures.containsKey(seatId)) {
            for (String feature : seatFeatures.get(seatId)) {
                switch (feature) {
                    case "heated":
                        seat = new HeatedSeatDecorator(seat);
                        break;
                    case "massage":
                        seat = new MassageSeatDecorator(seat);
                        break;
                }
            }
        }
